...
class ChannelAttnBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.channel_attn:
            ...
            # 修改通道注意力为CBAM
            self.se_channel_attn = CBAM(gate_channels=in_channels, reduction_ratio=reduction)
...
class SelfAttentionBlock(nn.Module):
    """
    专门用于平行处理的自注意力块
    处理图像格式的输入 [batch, channels, height, width]
    """
    ...
    def __init__(
        ...
        self.self_attn = CrossAttention(
            ...
            context_dim=None,  # 自注意力，context为None
            heads=n_heads, 
            dim_head=d_head, 
            dropout=dropout
        )
        ...
        # 前馈网络
        self. ff = FeedForward(self.inner_dim, dropout=dropout, glu=gated_ff)
        self.norm_ff = nn.LayerNorm(self.inner_dim)
        
        # 输出投影层 - 从token空间回到图像空间
        self.proj_out = nn.Conv2d(self.inner_dim, in_channels, kernel_size=1, stride=1, padding=0)
        
        print(f"Created SelfAttentionBlock: {in_channels} channels, {n_heads} heads, {d_head} head_dim")
    ...
    def forward(self, hidden_states):
        ...
        # 2. 投影到token空间
        hidden_states = self.proj_in(hidden_states)  # [batch, inner_dim, height, width]
        
        # 3. 重塑为序列格式
        hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(batch, height * width, self.inner_dim)
        
        # 4. 自注意力
        hidden_states = self.self_attn(hidden_states, context=None) + hidden_states
        
        # 5. 前馈网络
        hidden_states = self.ff(self.norm_ff(hidden_states)) + hidden_states
