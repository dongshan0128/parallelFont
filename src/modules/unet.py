...
from .unet_blocks import (DownBlock2D,
                          ...
                          get_down_block,
                          get_up_block,
                          get_up_block_with_parallel,
                          get_down_block_with_parallel)
...
class UNet(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block_with_parallel(
                down_block_type,
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            # 根据块类型和设计意图决定上采样
            if is_final_block:
                add_upsample = False  # 最终层不上采样
            elif input_channel == output_channel:
                add_upsample = False  
            else:
                add_upsample = True   # 其他层正常上采样
    ...
    def forward(
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # 获取对应的中间状态，这些中间状态是由与上采样块同层的下采样块产生的
            ...
            # ✅ 改进条件判断
            if hasattr(upsample_block, "sc_interpreter_offsets") and upsample_block.sc_interpreter_offsets is not None:
                ...
                # 处理不同类型的上采样块
                if hasattr(upsample_block, 'parallel') and upsample_block.parallel:
                    # 平行处理块
