...
def get_args():
    ...
    parser.add_argument("--resume_from_checkpoint", type=str, default=None,
                        help="Path to checkpoint directory (e.g., outputs/xxx/global_step_240000)")
...
def main():
    ...
    if args.resume_from_checkpoint:
        ...
        # 加载模型权重
        unet.load_state_dict(torch.load(f"{checkpoint_dir}/unet.pth", map_location="cpu"))
        style_encoder.load_state_dict(torch.load(f"{checkpoint_dir}/style_encoder.pth", map_location="cpu"))
        content_encoder.load_state_dict(torch.load(f"{checkpoint_dir}/content_encoder.pth", map_location="cpu"))
        ...
        if "global_step_" in dir_name:
            ...
            if accelerator.is_main_process:
                print(f"  ✓ Loaded weights from step {resume_global_step}")
        
        if accelerator.is_main_process:
            print(f"{'='*60}\n")
    elif args.phase_2:
        # Phase 2 训练：加载 Phase 1 权重
    ...
    if resume_global_step > 0:
        for _ in range(resume_global_step * args.gradient_accumulation_steps):
            lr_scheduler.step()
        if accelerator.is_main_process:
            print(f"📊 Resumed training state:")
            print(f"  - Starting from step:  {resume_global_step}")
            print(f"  - Remaining steps: {args.max_train_steps - resume_global_step}")
            print(f"  - Current learning rate: {lr_scheduler. get_last_lr()[0]:.2e}\n")
    ...
    if accelerator.is_main_process:
        ...
        print(f"  - Final step: {global_step}/{args.max_train_steps}")
        print(f"{'='*60}\n")
